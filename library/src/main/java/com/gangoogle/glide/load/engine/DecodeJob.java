...
import com.gangoogle.glide.GlideContext;
...
import com.gangoogle.glide.util.pool.FactoryPools.Poolable;
import com.gangoogle.glide.util.pool.GlideTrace;
...
class DecodeJob<R>
  ...
  private DataFetcherGenerator getNextGenerator() {
    switch (stage) {
      case RESOURCE_CACHE:
        return new ResourceCacheGenerator(decodeHelper, this, baseKey);
      case DATA_CACHE:
        return new DataCacheGenerator(decodeHelper, this, baseKey);
      case SOURCE:
        return new SourceGenerator(decodeHelper, this, baseKey);
      case FINISHED:
        return null;
      default:
  ...
  <Z> Resource<Z> onResourceDecoded(DataSource dataSource, @NonNull Resource<Z> decoded) {
    ...
    if (diskCacheStrategy.isResourceCacheable(
      ...
      deferredEncodeManager.init(key, baseKey, encoder, lockedResult);
      result = lockedResult;
    }
    return result;
  }
  ...
  private static class DeferredEncodeManager<Z> {
    ...
    @SuppressWarnings("unchecked")
    <X> void init(Key key, BaseKey baseKey, ResourceEncoder<X> encoder,
    ...
    void encode(DiskCacheProvider diskCacheProvider, Options options) {
      ...
      try {
        diskCacheProvider
            .getDiskCache()
            .put(baseKey, new DataCacheWriter<>(encoder, toEncode, options));
      } finally {
        toEncode.unlock();
        GlideTrace.endSection();
      }
